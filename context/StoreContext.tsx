    ...
    async (
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          addToast('success', 'Product Added!', `"${result.data.name}" is now live in the catalog.`);
          return { success: true, message: 'Product added successfully', data: result.data };
        } else {
          const errMsg = result.error || 'Failed to add product.';
          addToast('error', 'Add Failed', errMsg);
          return { success: false, message: errMsg };
        }
      } catch (err) {
        addToast('error', 'Network Error', 'Could not create product.');
  ...
  const deleteProduct = useCallback(
    async (
      ...
      try {
        ...
        if (!target) {
          addToast('error', 'Product Not Found', `No product matches code "${productId}".`);
          return { success: false, message: `No product found with code "${productId}".` };
        }
        ...
        if (productNameVerification !== target.name) {
          addToast(
            'error',
            'Verification Failed',
            'Entered product name does not match the exact case-sensitive name.'
          );
          return {
            success: false,
        ...
        if (result.success) {
          ...
          addToast('success', 'Product Deleted', `"${target.name}" (${productId}) was deleted.`);
          return { success: true, message: `Product "${target.name}" was successfully deleted.` };
        } else {
          const errMsg = result.error || 'Failed to delete product on server.';
          addToast('error', 'Delete Failed', errMsg);
          return { success: false, message: errMsg };
        }
      } catch (err) {
        addToast('error', 'Network Error', 'Could not delete product.');
        return { success: false, message: 'Network error occurred.' };
      }
    },
    [products, addToast, quickViewProduct]
  );
